...
void Brick::render()
{
	...
	else
	{
		...
		{ // Rendre tant que l'animation n'est pas terminée
...
bool Brick::isTouchedByBall(Ball *ball)
{
	...
	if (!isDestroyed())
	{
		...
		result = x >= this->getTextureWithPosition()->getX() - halfBallSize && x <= this->getTextureWithPosition()->getX2() + halfBallSize && y >= this->getTextureWithPosition()->getY() - halfBallSize && y <= this->getTextureWithPosition()->getY2() + halfBallSize;
	}
	return result;
}

bool Brick::isTouchedByMissile(Brick *missileBrick)
{
	bool result = false;
	if (!isDestroyed())
	{
		SDL_Rect missileRect = missileBrick->getTextureWithPosition()->getPosition();
		missileRect.w = static_cast<int>(16.0f); // Taille du missile (MISSILE_SIZE)
		missileRect.h = static_cast<int>(16.0f);
		SDL_Rect brickRect = this->getTextureWithPosition()->getPosition();
		result = SDL_HasIntersection(&missileRect, &brickRect);
	}
	return result;
}
...
void Brick::bounces(Ball *ball)
{
	...
	{ // Collision principalement horizontale (gauche ou droite)
