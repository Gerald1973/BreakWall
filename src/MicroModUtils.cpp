...
void MicroModUtils::vibrato(channel *chan) {
	chan->vibrato_add = waveform(chan->vibrato_phase, chan->vibrato_type)
			* chan->vibrato_depth >> 7;
}
void MicroModUtils::tremolo(channel *chan) {
	chan->tremolo_add = waveform(chan->tremolo_phase, chan->tremolo_type)
			* chan->tremolo_depth >> 6;
}
void MicroModUtils::trigger(channel *channel) {
	...
	if (channel->note.key > 0) {
		period = (channel->note.key * fine_tuning[channel->fine_tune & 0xF])
				>> 11;
...
void MicroModUtils::channelRow(struct MicroModUtils::channel *chan) {
	...
	chan->vibrato_add = chan->tremolo_add = chan->arpeggio_add =
			chan->fx_count = 0;
...
void MicroModUtils::resample(channel *chan, short *buf, long offset,
		long count) {
	unsigned long epos;
...
long MicroModUtils::initialise(unsigned char data[], long sampling_rate) {
	...
	for (inst_idx = 1; inst_idx < 32; inst_idx++) {
		...
		sample_length = unsignedShortBigEndian(module_data, inst_idx * 30 + 12)
				* 2;
		...
		loop_start = unsignedShortBigEndian(module_data, inst_idx * 30 + 16)
				* 2;
		loop_length = unsignedShortBigEndian(module_data, inst_idx * 30 + 18)
				* 2;
...
long MicroModUtils::calculateSongDuration() {
...
void MicroModUtils::setGain(long value) {
	gain = value;
}
