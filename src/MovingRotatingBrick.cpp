...
void MovingRotatingBrick::performEvent(SDL_Event &event)
{
	...
	else if (event.user.code == CustomEventUtils::Code::MISSILE_MOVED && !isDestroyed() && !isMoving)
	{
...
void MovingRotatingBrick::determineNearestEdge()
{
	...
	{
		moveDirX = -1.0f;
		moveDirY = 0.0f; // Move left
	}
	else if (minDist == distRight)
	{
		moveDirX = 1.0f;
		moveDirY = 0.0f; // Move right
	}
	else if (minDist == distTop)
	{
		moveDirX = 0.0f;
		moveDirY = -1.0f; // Move up
	}
	else
	{
		moveDirX = 0.0f;
		moveDirY = 1.0f; // Move down
	}
}
